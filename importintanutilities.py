...
# Define find_channel_in_signal_group function
def find_channel_in_signal_group(channel_name, signal_group):
...
def find_channel_in_header(channel_name, header):
    ...
    channel_found, channel_index = find_channel_in_signal_group(channel_name, header['amplifier_channels'])
    if channel_found:
        return True, 'amplifier_channels', channel_index
        
    # 2. Look through aux_input_channels
    channel_found, channel_index = find_channel_in_signal_group(channel_name, header['aux_input_channels'])
    if channel_found:
        return True, 'aux_input_channels', channel_index
    
    # 3. Look through supply_voltage_channels
    channel_found, channel_index = find_channel_in_signal_group(channel_name, header['supply_voltage_channels'])
    if channel_found:
        return True, 'supply_voltage_channels', channel_index
    
    # 4. Look through board_adc_channels
    channel_found, channel_index = find_channel_in_signal_group(channel_name, header['board_adc_channels'])
    if channel_found:
        return True, 'board_adc_channels', channel_index
    
    # 5. Look through board_dig_in_channels
    channel_found, channel_index = find_channel_in_signal_group(channel_name, header['board_dig_in_channels'])
    if channel_found:
        return True, 'board_dig_in_channels', channel_index
    
    # 6. Look through board_dig_out_channels
    channel_found, channel_index = find_channel_in_signal_group(channel_name, header['board_dig_out_channels'])
    if channel_found:
...
def plot_channel(channel_name, result):
    ...
    if channel_found:
        ...
        ax.set_ylabel('Voltage (microVolts)') # To do - update for different signal groups
        ax.plot(result['t_amplifier'], result['amplifier_data'][signal_index,:]) # To do - associate amplifier_channels with amplifier_data
        ax.margins(x=0, y=0)
        
    else:
        print('Plotting not possible; no channel with name ', channel_name, ' found')
